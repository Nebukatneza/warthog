...
case class Clause(entry: Set[FOLLiteral]) {
  ...
  def substitute(varmap:Map[FOLVariable,FOLTerm]):Option[Clause]={
    Clause.instanciate(entry.toList,varmap) match{
      case None => None
...
object Clause{
  ...
  def instanciate(lits:List[FOLLiteral],varmap:Map[FOLVariable,FOLTerm]):Option[List[FOLLiteral]]={
  ...
  def instanciateHelper(lits:List[FOLLiteral],varmap:Map[FOLVariable,FOLTerm]):(List[FOLLiteral],Boolean)={
        ...
        if (newpred.symbol.equals(PredicateSymbol("=",2))&& newpred.args.head.isInstanceOf[FOLFunction]&&newpred.args.tail.head.isInstanceOf[FOLFunction]&&
        isConstant(newpred.args.head.asInstanceOf[FOLFunction])&&isConstant(newpred.args.tail.head.asInstanceOf[FOLFunction]))
          {val a = newpred.args.head
           val b = newpred.args.tail.head
            if(a.equals(b))
            { if (lits.head.phase){
              return (List[FOLLiteral](),true)}
              else{return (List[FOLLiteral]()++rekurs._1,false)}

            }else{

              if(lits.head.phase){
              return (List[FOLLiteral]()++rekurs._1,false)
              }else{return (List[FOLLiteral](),true)}}
          }
